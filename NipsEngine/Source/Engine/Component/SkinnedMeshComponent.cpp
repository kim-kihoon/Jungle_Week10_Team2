...
{
	if (!HasValidMesh() || RenderVertices.empty() || SkinningMatrices.empty()) return;
...
const TArray<FNormalVertex>& USkinnedMeshComponent::GetRenderVertices() const
{
	return RenderVertices;
}

const TArray<uint32>& USkinnedMeshComponent::GetRenderIndices() const
{
	static const TArray<uint32> Empty;
	return SkeletalMeshAsset ? SkeletalMeshAsset->GetIndices() : Empty;
}

const TArray<FSkeletalMeshSection>& USkinnedMeshComponent::GetSections() const
{
...
void USkinnedMeshComponent::UpdateRenderVertices(ID3D11DeviceContext* InContext, const TArray<FNormalVertex>& InVertices)
{
...
void USkinnedMeshComponent::RebuildMeshBuffer()
{
	MeshBuffer.Release();
