	...
	FLineBatcher* LineBatcher)
{
	...
	case EPrimitiveType::EPT_SkeletalMesh:
	{
		...
		for (int32 SectionIdx = 0; SectionIdx < static_cast<int32>(Sections.size()); ++SectionIdx)
		{
			...
			Cmd.Type = ERenderCommandType::StaticMesh;
			Cmd.MeshBuffer = MeshBuffer;
			Cmd.SectionIndexStart = Section.FirstIndex;
			Cmd.SectionIndexCount = Section.GetIndexCount();
			Cmd.Material = Material;
			...
			if (Material->GetEffectiveLightingModel() == ELightingModel::Toon)
			{
				...
				OutlineCmd.SectionIndexStart = Section.FirstIndex;
				OutlineCmd.SectionIndexCount = Section.GetIndexCount();
				OutlineCmd.Material = Material;
